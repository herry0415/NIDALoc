...
parser.add_argument('--log_dir', default=f'/data/drj/Snail_Radar_weights/NIDALoc_{SEQUENCE_NAME}_Lidar/',
                    help='Log dir [default: log]')#todo
parser.add_argument('--dataset_folder', default=f'/data/drj/snail-radar/',
...
parser.add_argument('--resume_model', type=str, default='checkpoint_epoch90.tar',
...
def valid_one_epoch(model, val_loader, val_writer, device):
    ...
    for step, (val_data, val_pose, val_grid, val_hd) in enumerate(val_loader):
        ...
        pcs_tensor                           = val_data.to(device)    #todo                 
    ...
    image_filename = os.path.join(os.path.expanduser(FLAGS.log_dir), f'epoch_{args["resume_epoch"]}_lidar_{SUB_SEQUENCE_NAME}_{SEQUENCE_NAME}.png') #todo 注意改radar/lidar
