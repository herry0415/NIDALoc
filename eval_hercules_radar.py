...
SEQUENCE_NAME = 'Library' #['Library', 'Mountain', 'Sports'] #todo
...
parser.add_argument('--resume_model', type=str, default='checkpoint_epoch91.tar',
                    help='If present, restore checkpoint and resume training')#todo 改后缀以及后面的valid文件
...
LOG_FOUT = open(os.path.join(FLAGS.log_dir, f'epoch_{args["resume_epoch"]}_log.txt'), 'w') #todo
...
if FLAGS.normalize:
    if FLAGS.dataset == 'NCLT':
        ...
        stats_file = os.path.join(FLAGS.dataset_folder, sequence_name, sequence_name + '_radar' + '_pose_stats.txt') #改为radar
    else:
...
sequence_name = SEQUENCE_NAME #todo
...
val_set      = MF(**valid_kwargs) #todo 在MF里面 进行更改
...
def valid_one_epoch(model, val_loader, val_writer, device):
    ...
    for step, (val_data, val_pose, val_grid, val_hd) in enumerate(val_loader):
        ...
        pcs_tensor                           = val_data.to(device, dtype=torch.float32)    #todo                 
    ...
    image_filename = os.path.join(os.path.expanduser(FLAGS.log_dir), f'epoch_{args["resume_epoch"]}_radar_{SEQUENCE_NAME}.png') #todo 注意改radar/lidar
